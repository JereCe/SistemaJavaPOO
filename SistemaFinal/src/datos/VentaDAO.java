...
public class VentaDAO implements CrudVentaInterface<Venta, DetalleVenta>{
    ...
    public List<DetalleVenta> listarDetalle(int id) {
        ...
        try {
            ps = CON.conectar().prepareStatement("SELECT a.id, a.codigo,.a.nombre,a.stock,d.cantidad,d.precio,d.descuento,(d.cantidad*d.precio-d.descuento) as sub_total  FROM detalle_venta d INNER JOIN articulo a ON d.articulo_id = a.id WHERE d.venta_id=?");
            ps.setInt(1,id);
            rs = ps.executeQuery();
            while (rs.next()) {
